...
/// 历史状态判定——那是 <see cref="MarkerTrackingSession"/> 的职责(design D1)。
/// </summary>
[AddComponentMenu("MR Base/Native/PICO Fiducial Observation Source")]
[DisallowMultipleComponent]
public sealed class PicoFiducialObservationSource : MonoBehaviour, IMarkerObservationSource
{
    [Header("Performance")]
    ...
    private readonly List<MarkerObservation> pendingObservations = new List<MarkerObservation>();
    ...
    /// <summary>启动企业服务与相机绑定流程。幂等——重复调用不会重新初始化。</summary>
    public void Open()
    {
        if (opened)
        {
        ...
        if (!PXR_Enterprise.InitEnterpriseService(true))
        {
            ...
            Debug.LogError("[PicoFiducialObservationSource] InitEnterpriseService failed");
            return;
        }
    ...
    /// <summary>释放相机与企业服务(task 3.6:只有 Close 才释放,Pause 不释放)。</summary>
    public void Close()
    {
        if (!opened)
        {
        ...
        serviceBound = false;
        Debug.Log(
            $"[PicoFiducialObservationSource] stop; sampled={sampledFrames} dropped={droppedFrames} " +
            $"detect={detectSuccesses}/{detectAttempts}");
    }

    /// <summary>只跳过 DispatchDetection;相机流与 4U 会话保持不变(0.1 的结论,task 3.6)。</summary>
    public void Pause() => paused = true;

    public void Resume() => paused = false;

    /// <summary>返回自上次调用以来新完成的检测批次;不做保持显示或历史判定(design D1)。</summary>
    public IReadOnlyList<MarkerObservation> Poll()
    {
        lock (gate)
        {
            ...
            {
                return Array.Empty<MarkerObservation>();
            }

            var snapshot = new List<MarkerObservation>(pendingObservations);
    ...
    {
        serviceBound = bound;
        Debug.Log($"[PicoFiducialObservationSource] BindEnterpriseService={bound}");
        if (!bound) return;
    ...
    private void OnCameraOpened(bool opened_)
    {
        cameraStarting = false;
        cameraOpen = opened_;
        pendingCameraStart = opened_;
        Debug.Log($"[PicoFiducialObservationSource] OpenCameraAsyncfor4U={opened_}");
    }
    ...
    private void StartCameraStream()
    {
        ...
        {
            Debug.LogWarning("[PicoFiducialObservationSource] GetCameraExtrinsicsfor4U 失败（只影响日志，不参与合成）");
        }
        ...
        Debug.Log(
            $"[PicoFiducialObservationSource] detector ready {requestedWidth}x{requestedHeight} " +
    ...
    private void Update()
    {
        ...
        // Pause() 只跳过 DispatchDetection,相机流与采样时钟继续走(0.1 的结论)。
        if (paused) return;
        ...
        ThreadPool.QueueUserWorkItem(_ =>
        {
            try
            {
                ...
                if (attempts % 30 == 0)
                {
                    ...
                    Debug.Log(
                        $"[PicoFiducialObservationSource] detect {hits}/{attempts} = {(100.0 * hits / attempts):F0}% " +
            ...
            catch (Exception exception)
            {
                ...
                Debug.LogWarning($"[PicoFiducialObservationSource] detect failed: {exception}");
            }
            finally
            {
    ...
    private void DrainResults()
    {
        while (detectResults.TryDequeue(out DetectedMarker detected))
        {
            ...
            if (detected.HasPose)
            {
                lock (gate)
                {
                    pendingObservations.Add(new MarkerObservation(
                        MarkerPlatform.Pico, detected.Id.ToString(), detected.WorldPose));
                }

                Debug.Log(
                    $"[PicoFiducialObservationSource] tag {detected.Id} hamming={detected.Hamming} " +
                    $"margin={detected.DecisionMargin:F1} pos={detected.WorldPose.position:F3} " +
                    $"rot={detected.WorldPose.rotation.eulerAngles:F1}");
            }
            else
            {
                Debug.Log($"[PicoFiducialObservationSource] tag {detected.Id} 检出但无位姿");
            }
        }
    }
