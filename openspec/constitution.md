...
1. **收敛理由成文且可反驳**。必须给出一个**具名的可计算量**，并写出它在「合」与「拆」两种形态下的数值，
   证明拆开会让这个量变差。**不接受**「集中管理更清晰」「放一起更好维护」这类无法计算的说法。

   合法的量举例（不限于这两个，但必须同样能算）：
   - **新增一个同类变体时需改动的文件数**。`PlatformRuntime` 适用此量：三条轴各自都带 `#if MRBASE_*`
     （全文 8 处），合则加第三个平台改 **1** 个文件，拆则改 **3** 个。
...
① 可计算量 = **新增一个平台要改的分叉文件数**，合 = 1，拆 = 3（三条轴各自都带 `#if MRBASE_*`）；
