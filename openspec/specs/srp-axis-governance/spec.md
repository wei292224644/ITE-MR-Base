# srp-axis-governance Specification

## Purpose

规定本仓如何判定单一职责（SRP）违反：以**变更轴计数**为唯一度量，明确违反阈值、五条豁免的适用边界、
`design.md` 的机械可检查门槛、存量违反的 waiver 承载方式，以及 `docs/architecture/srp-audit.md`
审计清单的覆盖与取证要求。
