...
- [ ] 3.1 `IteRuntime.LoadAsync` 在 `RequireScan()` 之前调一次 `_assembler.SetAllVolumesActive(true)`——**默认开启**，宿主不需要为了让区域触发工作而调任何东西（design D2）
- [ ] 3.2 `IteRuntime` 暴露 `SetTriggerVolumesActive(bool)`，转发给 assembler，供宿主延后开启或临时关闭（例如加载页仍覆盖视野时）
