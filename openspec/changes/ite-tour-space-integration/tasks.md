...
- [x] 5.2 假扫码驱动：**由驱动层构造** `MockObservationSource` 与 `MarkerTrackingSession`，每帧 `Tick(Time.deltaTime)`，并在 `Start()` 里调 `IteHostBootstrap.AttachMarkerSession` 把会话注进去（依赖方向：输入层 → 装配层，反向不成立）
