...
**替代方案与否决理由**
- **行数**：本仓有现成反例——`BuildScript.cs` 640 行只有一条坐实轴「出包配置变」，拆了反而把
  一条流程散到多个文件；`IteHostBootstrap.cs` 350 行六条候选轴。行数与轴数不相关，甚至反向。
- **方法数 / 圈复杂度**：它们度量「难读」。本次要解决的是「意图不清」，而意图不清的根因是
  一个类被多个不相干的理由驱动——那正是变更轴，不是复杂度。
- **决定性理由**：变更轴**可证伪**。一条轴必须有可引用的凭据才算坐实，否则是臆想的轴，不计数。
  这让审计排名能被反驳，而不是 AI 的主观排序。凭据的三种形态（历史 / 消费面 / 实测记录）
  见 D12——原本只认 git 历史，实施中发现那会让判据在本仓自废。
...
- 第二遍**候选深读**（≥2 轴者）：每条轴要么拿到三类凭据之一（见 D12），要么标 `[推测轴]`。
  坐实不了的轴从排序用的计数里划掉。
...
① 给出一个**具名可计算量**及其在「合」「拆」两形态下的数值（不接受「集中管理更清晰」）；
...
「全部平台分叉的唯一落点」「do not add new `#if MRBASE_*` elsewhere」。可计算量是
**新增一个平台要改的分叉文件数**：合 = 1，拆 = 3（三条轴各自都带 `#if MRBASE_*`）。
拆开还直接违反既有架构决定。若判据把这种「刻意汇聚」与
...
它要求论证是**可反驳的**；第 ③ 条保证豁免不产生信息黑洞。

**条件 ① 的措辞在 apply 任务 1.7 中被修正**：原文钉死在「轴数 × 承载文件数」这一个公式上，
实测发现它对等分拆分不敏感——`PlatformRuntime` 合为 1 文件 3 轴 = 3，拆为 3 文件各 1 轴 = 3，
数值相同，无法区分两种形态。真正有区分力的量是「新增一个平台要改的分叉文件数」（合 1、拆 3）。
故条件 ① 改为「给出一个具名可计算量，并写出它在合与拆两形态下的数值」，并附两个示例量与
各自的适用提醒。
...
### D12 — 坐实证据不止 git 历史：加消费面与实测记录两类（实施中修正）

**触发**：apply 任务 3.2 一开跑就撞墙。`IteHostBootstrap` 六条轴的引入提交只有两个
（`df057ec` 带相机解析/网络判定/佩戴状态，`0b71471` 带会话注入/事件转发/超时），
没有任何一条轴被**单独**触发过。`IteTourObject` 四条轴塌进 3 个提交，`TourDirector` 四条塌进 2 个。

**根因**：仓库太年轻——2026-07-27 起 236 个提交，候选类各自只改过 3–7 次，代码基本一次写成。
按原规则（只认「单独触发的提交」），坐实轴数普遍为 0，排序公式「坐实轴数 × 消费者数」全体归零，
连 `IteHostBootstrap` 都会被判成不违反。**git 的沉默不是反证，只说明代码还没被改第二次。**

**选了什么**：坐实的凭据扩为三类，满足任一即可——① git 单独触发的提交；
② **独立消费者只取用该轴的 public 面**；③ 代码注释或 design 里的独立实测记录（含日期与现象）。

**为什么②是合法证据，而且比①更硬**：四个消费者各自只依赖 `IteHostBootstrap` 一条轴的表面
（`IteEditorHud`→`Runtime`+`MarkerBridge`、`IteEditorFakeScan`→`AttachMarkerSession`、
`IteDeviceMarkerRig`→`StartRuntimeAsync`、`IteHmdPanel`→`Failed`）。这证明这些轴**此刻**就在被
独立使用，是现在时、可随时复核的事实；git 证据反而是过去时，且受仓库年龄影响。

**替代方案与否决理由**
- **(B) 坐实规则不动，排序改用「候选轴数 × 消费者数」**：改动最小，但「坐实」这个概念随即作废，
  判据退回 AI 主观排名——正是 D1 要避免的。否决。
- **(C) 按「轴簇数」（轴被引入的不同提交数）排序**：纯历史驱动，度量的是**写代码的批次**，
  与职责无关。否决：没有结构含义。
