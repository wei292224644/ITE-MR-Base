...
| 16 | **XR Hands 1.8.1 在 Unity 6 上注入不了 `oculus.software.handtracking`** —— 它只实现了旧的 `OnPostGenerateGradleAndroidProjectExt`，而 Unity 6 的内部 Android 构建管线走 XR Management 4.4 的 `ProvideManifestRequirementExt`（对照 OpenXR 包的 `ModifyAndroidManifestMeta.cs`：两个都实现了，所以 `vr.headtracking` 进得去）。后果是 APK 不声明手部追踪，Quest 在无手柄时直接拦截启动，logcat 报 `common_system_dialog_app_launch_blocked_controller_required`。本项目全程手势交互，故自建 `HandTrackingManifestFix` 补注入，等上游修复后删除 | `com.unity.xr.hands@1.8.1/Editor/OpenXR/ModifyAndroidManifest.cs:28` vs `com.unity.xr.openxr@1.16.1/Editor/MetaQuest/ModifyAndroidManifestMeta.cs:45`；Quest 真机实测 |
